...
'''
在深度学习模型训练和评估过程中常用的辅助功能，具体包括：
计时装饰器：用于记录函数的运行时间。
模型保存与加载：提供保存和加载模型的功能，同时支持保存和恢复优化器状态。
最佳模型保存：根据验证损失或准确率保存最佳模型。
绘图功能：保存训练和验证的准确率及损失曲线。
配置文件加载：从 JSON 文件中加载配置信息。
目录创建：创建指定的目录。
表格写入：将数据写入 Excel 表格。
动态打印：在控制台动态刷新输出。
时间转换：将秒数转换为小时、分钟和秒的元组。
张量检查：检查张量是否包含 NaN 或过大的值。
参数计数：计算模型的可训练或总参数数量。
递归钩子：为模型的每个叶子模块注册前向钩子。
损失计算：计算模型在给定数据集上的损失。
'''
def timer(func):
