...
'''
utils.py 文件包含了一系列用于深度学习项目的实用工具函数和类，主要功能如下：
main.py  中初始化使用到该文件
配置设置：Setup 函数用于创建输出目录，保存配置信息到 JSON 文件，并返回配置字典。
目录创建：create_dirs 函数用于创建指定的目录，如果目录不存在的话。
初始化：Initialization 函数用于设置随机种子，选择计算设备（CPU 或 GPU）。
数据加载：Data_Loader 函数根据不同的问题类型调用相应的数据加载器。
数据集类：dataset_class 类继承自 torch.utils.data.Dataset，用于封装数据和标签，支持数据的索引访问和填充操作。
打印标题：print_title 函数用于打印带有边框的标题。
频率转换：convert_frequency 和 get_fft 函数用于将 EEG 数据转换到频域，并提取不同频段的特征。
'''
